...
from tlkit.models.lifelong_framework import LifelongSidetuneNetwork, ResnetiCifar44NoLinear, ResnetiCifar56NoLinear
...
model = LifelongSidetuneNetwork(
    base_class='ResnetiCifar44NoLinear',  # Base model sans le side_model
    base_kwargs={ 'eval_only': False },  # Mettre à False pour entraîner le base_model
    base_weights_path='../side-tuning/assets/pytorch/resnet44-cifar.pth',
    transfer_class="nn.Identity",
    dataset='icifar'
)
model.to(device)
...
tasks = [tasks[i] for i  in model.tasks]
